...
3a. **多组件问题必须分别搜**。如果用户问的是 "从 X 到 Y 的逐项分解"
    或 "X 的 A、B、C 各是多少" 这种**多个独立子项**的问题，第一次
    search 通常只能命中其中 1-2 项，**必须再发起 1-2 次 search**
    专门去找其余子项；只要还有缺失的字段，就不要直接回 NO_MATCH。
    例：用户问 "总收入、收入成本、销售开支、研发开支、净利润 各是多少"
        → 第 1 次：search "总收入 营业收入"
        → 第 2 次：search "收入成本 经营成本"
        → 第 3 次：search "销售开支 一般行政开支 研发开支"
    取每次的最相关数字，组合成完整答案
